...
async function createWindow() {
  ...
  ipcMain.handle(
    "file:write",
    async (_e, args: { path: string; content: string }) => {
      try {
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: "separator" },
        {
          label: "Preview",
          accelerator: "Alt+1",
          click: () => win.webContents.send("ui:setPanel", "preview"),
        },
        {
          label: "Metadata",
          accelerator: "Alt+2",
          click: () => win.webContents.send("ui:setPanel", "info"),
        },
        {
          label: "Diagnostics",
          accelerator: "Alt+3",
          click: () => win.webContents.send("ui:setPanel", "diagnostics"),
        },
      ],
    },
    {
      ...
      submenu: [
        ...
        { type: "separator" },
        {
          ...
          click: () => {
            ...
            if (!existsSync(aboutPath) && existsSync(candidate2))
              aboutPath = candidate2;
            if (!existsSync(aboutPath) && existsSync(candidate3))
              aboutPath = candidate3;
            ...
            aboutWin
              ...
              .catch((err) =>
                console.error("[main] failed to load about.html", err),
              );
            ...
            aboutWin.once("ready-to-show", () => {
              try {
                if (parent) {
                  ...
                  const x =
                    pBounds.x + Math.round((pBounds.width - aBounds.width) / 2);
                  const y =
                    pBounds.y +
                    Math.round((pBounds.height - aBounds.height) / 2);
